...
function PitcherBadge({ short, size = 44 }) {
  ...
  return (
    <div style={{ width: size, height: size, borderRadius: '22%', background: `${color}28`, border: `2px solid ${color}`, display: 'flex', alignItems: 'center', justifyContent: 'center', fontFamily: "'Barlow Condensed',sans-serif", fontWeight: 900, fontSize: size * 0.25, color, flexShrink: 0, letterSpacing: '.02em' }}>
      {short?.slice(0, 4)}
    </div>
  )
}
...
export default function GameCard({ game, isNext = false }) {
  const [modal, setModal] = useState(null) // 'boxscore' | 'recap' | 'gameday'
            ...
            {[
              ...
              <div key={side.t} style={{ display: 'flex', alignItems: 'center', marginBottom: i === 0 ? 4 : 0 }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 8, width: 140, flexShrink: 0 }}>
                  ...
                  <div>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 3 }}>
                      <Link to={`/teams/${side.team?.id || side.t}`} style={{ fontFamily: "'Barlow Condensed',sans-serif", fontWeight: side.won ? 800 : 400, fontSize: 22, textTransform: 'uppercase', color: side.won ? 'var(--white)' : 'var(--muted)', textDecoration: 'none', lineHeight: 1 }}>{side.t}</Link>
                ...
                {[side.score, side.he?.h, side.he?.e].map((val, vi) => (
                  <span key={vi} style={{ width: 52, textAlign: 'center', fontFamily: "'Barlow Condensed',sans-serif", fontWeight: vi === 0 && side.won ? 800 : 400, fontSize: 36, lineHeight: 1, color: vi === 2 ? 'var(--muted)' : side.won ? 'var(--white)' : 'var(--muted)' }}>{val}</span>
                ))}
              </div>
            ))}
          ...
          <div style={{ display: 'flex', flexDirection: 'column', justifyContent: 'center', padding: '16px 20px', borderLeft: '1px solid var(--border)', flex: 1, gap: 12, minWidth: 200 }}>
            {[{ role:'WIN', p:winP, short:winTeam }, { role:'LOSS', p:lossP, short:lossTeam }].map(({ role, p, short }) => (
              <div key={role} style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                ...
                <div>
                  <div style={{ fontWeight: 700, fontSize: 14, color: 'var(--white)', lineHeight: 1.2 }}>{p?.name}</div>
